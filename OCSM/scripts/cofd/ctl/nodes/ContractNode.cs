...
public partial class ContractNode : MarginContainer
{
	...
	public void ClearInputs()
	{
		...
		skillInput.Deselect();
		GetNode<ContractRegaliaOptionButton>(NodePaths.RegaliaInput).Deselect();
	...
	public Contract GetData()
	{
		...
		var regaliaNode = GetNode<ContractRegaliaOptionButton>(NodePaths.RegaliaInput);
		var contractTypeNode = GetNode<ContractTypeButton>(NodePaths.ContractTypeInput);
		...
		var attribute = Attribute.KindFromString(attr1Node.GetSelectedItemText());
		var attributeContested = Attribute.KindFromString(attribute3Input.GetSelectedItemText());
		var attributeResisted = Attribute.KindFromString(attribute2Input.GetSelectedItemText());
		...
		if(regaliaObj is null && regalia.Equals(ContractRegalia.Goblin.Name))
	...
	public void SetData(Contract contract)
	{
		if(metadataManager.Container is CofdChangelingContainer)
		{
			...
			if(contract.Regalia is not null)
				GetNode<ContractRegaliaOptionButton>(NodePaths.RegaliaInput).SelectItemByText(contract.Regalia.Name);
			
			if(contract.ContractType is not null)
				GetNode<ContractTypeButton>(NodePaths.ContractTypeInput).SelectItemByText(contract.ContractType.Name);
	...
	{
		if(index == (long)ActionOptionButton.Action.Contested)
		{
			versus.Show();
		...
		if(index == (long)ActionOptionButton.Action.Resisted)
		{
	...
	{
		GetChildren()
			.Where(node => string.IsNullOrEmpty(node.GetNode<SeemingOptionButton>(NodePaths.SeemingInput).GetSelectedItemText())
		...
		SeemingBenefits = GetChildren()
					...
					.Select(node => new {
						seeming = node.GetNode<SeemingOptionButton>(NodePaths.SeemingInput).GetSelectedItemText(),
